...
def generate_test_report(results):
    ...
    for ts in json_data[TESTSUITES]:
        suite_time_taken=0
        test_suite = ET.SubElement(test_result, 'testsuite')
        test_suite.set(NAME, ts[NAME])
        test_suite.set(TESTS,str(ts[PASSED]+ts[FAILED]+ts[SKIPPED]))
        test_suite.set(FAILED, str(ts[FAILED]))
        test_suite.set(SKIPPED, str(ts[SKIPPED]))
        if ts[NAME] in test_suites_failed:
            test_suite.set(RESULT, FAILED)
        else:
            test_suite.set(RESULT, PASSED)
        for tc in ts[RESULT]:
            test_case = ET.SubElement(test_suite, 'testcase')
            test_case.set(NAME, tc[NAME])
            test_case.set('classname', ts[NAME])
            test_case.set('status', tc[RESULT])
            test_case.set(TIME, str(tc[TIME]))
            suite_time_taken+=tc[TIME]
            if tc[RESULT] == FAILED:
                failure = ET.SubElement(test_case, 'failure')
                failure.set('message',str(tc[EXCEPTION]))
