...
def generate_xpath_text(command):
    ...
    return "(//*[text()='{text}'])[position() = {index}]".format(
        text=command[ARGS][SUBJECT], index=index
    )
...
def generate_xpath_for_generic_attribute(command):
    ...
    if command[ARGS][INDEX] != "":
        index = command[ARGS][INDEX]
    return "(//*[@{attribute}='{text}'])[position() = {index}]".format(
        text=command[ARGS][SUBJECT], index=index, attribute=command[ARGS][ATTRIBUTE]
    )
    ...
    while mode_index < len(execute_modes):
        ...
        try:
            if mode == "NAME":
                ...
                raise Exception(
                    "[Error] Invalid mode found while parsing command : ", command
                )
        except Exception:
